/**************************************************************************
 ...
 @DESCRIPTION:  Ejemplo básico de uso del HMI CNMB5_I2C para ArduPLCs. Este ejemplo usa los botones en modo biestable.
...
void loop() {
  ...
  do {
    ...
    // Cambiamos el estado de los circulos en funcion del boton que presionamos (modo BIESTABLE)
    if (PressUp() == 1)     u8g.drawDisc( height / 2, RADIO, RADIO);
    if (PressDown() == 1)   u8g.drawDisc( height / 2, RADIO + 4 * RADIO + 2, RADIO);
    if (PressMiddle() == 1) u8g.drawDisc( height / 2, RADIO + 2 * RADIO + 1, RADIO - 2);
    if (PressLeft() == 1)   u8g.drawDisc( height / 2 - height / 5, RADIO + 2 * RADIO + 1, RADIO);
    if (PressRight() == 1)  u8g.drawDisc( height / 2 + height / 5, RADIO + 2 * RADIO + 1, RADIO);
...
// LAS SIGUIENTES FUNCIONES NOS PERMITEN USAR LOS BOTONES COMO INTERRUPTORES O BIESTABLES Y NO COMO PULSADORES.
boolean PressUp() {
  static boolean pressed = false;
  static boolean estadoAnterior = false;
  boolean estadoActual;

  estadoActual = HMI_IO.digitalRead(BT_UP);
...
boolean PressDown() {
  ...
  estadoActual = HMI_IO.digitalRead(BT_DOWN);
  if (estadoActual == 0 & estadoAnterior == 1) {
    pressed = pressed ^ true;
  }
  estadoAnterior = estadoActual;
...
boolean PressMiddle() {
  ...
  estadoActual = HMI_IO.digitalRead(BT_INTRO);
  if (estadoActual == 0 & estadoAnterior == 1) {
    pressed = pressed ^ true;
  }
  estadoAnterior = estadoActual;
...
boolean PressLeft() {
  ...
  estadoActual = HMI_IO.digitalRead(BT_LEFT);
  if (estadoActual == 0 & estadoAnterior == 1) {
    pressed = pressed ^ true;
  }
  estadoAnterior = estadoActual;
...
boolean PressRight() {
  ...
  estadoActual = HMI_IO.digitalRead(BT_RIGHT);
  if (estadoActual == 0 & estadoAnterior == 1) {
    pressed = pressed ^ true;
  }
  estadoAnterior = estadoActual;
